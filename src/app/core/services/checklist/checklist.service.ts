...
export class ChecklistService {
  ...
  obtenerChecklist(ticketId: number): Observable<Checklist> {
    return this.http.get<ResponseData<Checklist>>(`${this.API_URL}/ticket/${ticketId}`)
  ...
  actualizarActividad(tipo: TIPOCHECKLIST, actividadId: number, request: ActualizarActividadRequest): Observable<void> {
    ...
    const params = new HttpParams()
      .set('completada', completadaStr);

    console.log('Params:', params.toString());
    if (tipo === TIPOCHECKLIST.GENERAL) {
      return this.http.put<ResponseData<void>>(`${this.API_URL}/actividad/${actividadId}`, null, {params})
        .pipe(map(() => void 0));
    } else {
      return null;
    }
