...
export class ZonaService {
    ...
    actualizarZona(param: { id: number; nombre: string; activo: boolean }) {
        return this.httpClient.put<ResponseData<void>>(`${this.apiUrl}`, param, {headers: this.header}).pipe(
            tap(() => this.data = undefined as any)
        );
    }

    registrarZona(param: { nombre: string }) {
        console.log('Enviando zona:', param);
        return this.httpClient.post<ResponseData<void>>(`${this.apiUrl}`, param, {headers: this.header}).pipe(
            tap(() => this.data = undefined as any)
        );
    }

    eliminarZona(id: number) {
        return this.httpClient.delete<ResponseData<void>>(`${this.apiUrl}/${id}`).pipe(
            tap(() => this.data = undefined as any)
        );
    }
}
