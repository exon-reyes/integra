...
import { switchMap, throttleTime, distinctUntilChanged } from 'rxjs/operators';
...
export class WorktimeService {
    ...
    iniciarJornada(empleadoId: number, fotoBase64: string | null, unidadId: number): Observable<ResponseData<any>> {
        const body: any = { empleadoId, unidadId };
        if (fotoBase64) body.foto = fotoBase64;
        return this.enviarAccionProtegida('iniciar', body);
    }

    finalizarJornada(empleadoId: number, fotoBase64: string | null, unidadId: number): Observable<ResponseData<any>> {
        const body: any = { empleadoId, unidadId };
        if (fotoBase64) body.foto = fotoBase64;
        return this.enviarAccionProtegida('finalizar', body);
    }

    iniciarPausa(empleadoId: number, tipoPausa: TipoPausa, fotoBase64: string | null, unidadId: number): Observable<ResponseData<any>> {
        const body: any = { empleadoId, pausa: tipoPausa, unidadId };
        if (fotoBase64) body.foto = fotoBase64;
        return this.enviarAccionProtegida('pausa/iniciar', body);
    }

    finalizarPausa(empleadoId: number, tipoPausa: TipoPausa, fotoBase64: string | null, unidadId: number): Observable<ResponseData<any>> {
        const body: any = { empleadoId, pausa: tipoPausa, unidadId };
        if (fotoBase64) body.foto = fotoBase64;
